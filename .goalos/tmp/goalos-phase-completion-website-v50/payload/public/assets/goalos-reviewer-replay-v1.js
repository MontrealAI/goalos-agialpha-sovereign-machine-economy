(() => {
  ...
  function copyIssue(){ const text=`## External Reviewer Replay Room Review\n\nDecision: ${decision()}\nReadiness: ${score()}/100\n\nBoundary confirmed: no user data, no user funds, no wallet, no transaction.\n\nGates:\n${Object.entries(gates).map(([k,v])=>`- [${v?'x':' '}] ${k}`).join('\n')}\n`; if(navigator.clipboard){navigator.clipboard.writeText(text).then(()=>log('GitHub-ready review issue copied.')).catch(()=>log('Copy unavailable; download report instead.'));} else {log('Clipboard unavailable; download report instead.');}}
