...
TARGET → EMIT → FILTER → ATLAS → TEST-PLAN → EVAL → INSERT → PROMOTE. OMNI is search allocation / interestingness guidance only. Outcome authority requires risk, evidence, baseline, persistence, replay, and validator gates.
